...
- **Prevention:** Deny non-TLS S3; deny PHI `PutObject` without SSE-KMS; protect account Block-Public-Access. Even account admins cannot override.
- **Detection:** Org CloudTrail on policy changes.
- **Alert:** Policy change → change-alerter [40].

## What would trigger an alert

- An admin tries to upload PHI without SSE-KMS, or over plain HTTP → request denied → s3-access-denied / unauthorized-api alarm [35].
- The SCP itself is edited or detached → change-alerter [40] → SNS [36].
