...
| R4 | **Alerting** ("alarms for everything") | SNS `ith-security-alerts` + 12 CloudWatch metric-filter alarms (9 CIS-style [35] + 3 change/CreateUser/**detection-tampering** [40]) + GuardDuty->SNS (which also carries the R12 role-credential alerts); the detection stack is itself locked by the protect-detection SCP [60] (self-protection: prevent + detect). 3 alarms already firing on real activity. | DONE |
...
| R12 | **Role assumptions -> IP-based alerting** | Handled by **GuardDuty [37]**, not a bespoke Lambda. `InstanceCredentialExfiltration.OutsideAWS` detects EC2 role creds used from an IP **outside AWS**; `.InsideAWS` detects them used from **another AWS account** (which a static IP allow-list would miss). Both sev>=4 -> EventBridge -> SNS. Managed, no allow-list to maintain. (An earlier hand-rolled EventBridge-on-`AssumeRole` + `ith-ip-alerter` Lambda was removed as a duplicate of this.) | DONE |
| R13 | **3 users** + README login URL | `ITH-SuperAdmin` (all, capped by **permissions boundary [42]** to all-except-`kms:*`), `ITH-Admin` (scoped, **inline deny kms:***), `ITH-S3Reader` (read S3 **only in-VPC**). SuperAdmin & Admin both lose KMS by *different* levers (boundary ceiling vs explicit deny). Permission sets real; Entra users doc-only. Login URL on README p.1. | PARTIAL (perm-sets real / users doc-only) |
...
- **Root-account usage alerting** - root lives in the mgmt account we don't fully own (the
  member-account `root-usage` alarm [35] exists; org/mgmt-trail coverage is the gap).
- **Root credential removal + multi-party approval (Organizations)** - prod would centralize
  root access (delete member-account root creds) and add MPA so air-gapped backup recovery
  needs a separate approval team, resilient to root compromise. See OutOfScopeNotes.
- **AWS Config as an S3 guardrail** - considered for locking the bucket to the super admin;
  Config is **detective-only** (can't block a call), so enforcement stays an SCP/bucket-policy
  + RCP, with Config for drift-detection/auto-remediation. See OutOfScopeNotes.
- **Risk/behaviour-based sign-in** - Entra ID Protection risk-based CA; left doc-only under
  the tenant guardrail. AWS-side behaviour detection is already covered by GuardDuty [37]
  (incl. the `InstanceCredentialExfiltration` findings that satisfy R12).
...
- [x] `app/` - web app, redactor Lambda, vaultless tokenizer
